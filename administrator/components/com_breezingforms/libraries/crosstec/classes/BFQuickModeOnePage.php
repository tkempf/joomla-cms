...
');
		...
		if($this->toggleFields != '[]'){
			...
			function bfRegisterToggleFields(){
                ...
                JFactory::getDocument()->addScript(JURI::root(true) . '/components/com_breezingforms/libraries/jquery/ladda/ladda.jquery.min.js');
			...
			if($mdata['type'] == 'page'){
				...
				switch($mdata['bfType']){
					...
					case 'bfCalendarResponsive':
						...
						echo '<div class="input-append">'."\n";
						echo '<input autocomplete="off" class="ff_elem" '.$size.'type="text" name="ff_nm_'.$mdata['bfName'].'[]"  id="ff_elem'.$mdata['dbId'].'" value="'.htmlentities($left, ENT_QUOTES, 'UTF-8').'"/>'."\n";
						echo '<button style="cursor: pointer;" type="button" id="ff_elem'.$mdata['dbId'].'_calendarButton" type="submit" class="bfCalendar btn button" value="'.htmlentities($right, ENT_QUOTES, 'UTF-8').'"><span><i class="icon-calendar"></i>'.htmlentities($right == '...' ? '' : $right, ENT_QUOTES, 'UTF-8').'</span></button>'."\n";
						echo '</div>'."\n";
		...
		if(isset($dataObject['properties']) && $dataObject['properties']['type'] == 'section' && $dataObject['properties']['bfType'] == 'section'){
			...
			if(!$isLastPage){
				...
				if($this->rootMdata['pagingInclude'] && $dataObject['properties']['pageNumber'] < count($this->dataObject['children']) - $last){
                                        ...
                                        /* translatables end */
					echo '<button type="button" class="bfNextButton btn btn-primary pull-right button'.$this->fadingClass.'" type="submit" onclick="ff_currentpage = '.json_encode($dataObject['properties']['pageNumber']).';bf_validate_nextpage('.($dataObject['properties']['pageNumber'] + 1).');populateSummarizers();if(typeof bfRefreshAll != \'undefined\'){bfRefreshAll();}" value="'.htmlentities(trim($this->rootMdata['pagingNextLabel']), ENT_QUOTES, 'UTF-8').'"><span>'.htmlentities(trim($this->rootMdata['pagingNextLabel']), ENT_QUOTES, 'UTF-8').'</span></button>'."\n";
				}
		...
		echo '<noscript>Please turn on javascript to submit your data. Thank you!</noscript>'."\n";
                ...
                if( $this->rootMdata['lastPageThankYou'] ){
                    echo '
                        ...
                        <br>
                        <button data-remodal-action="confirm" class="btn btn-primary remodal-confirm">'.BFText::_('COM_BREEZINGFORMS_OK').'</button>
                      </div>
                      <script type="text/javascript">
                      <!--
                      JQuery(document).ready(function(){
